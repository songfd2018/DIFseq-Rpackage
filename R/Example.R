...
# alpha.syn[1:5] <- 4
# alpha.syn[G/4 + 1:5] <- 3
# alpha.syn[G/4 * 2 + 1:5] <- 2.5
# alpha.syn[G/4 * 3 + 1:5] <- 2
#
...
# beta.syn[1:5,2] <- -2
# beta.syn[6:10, 2] <- 2
# beta.syn[G/4 + 1:5,2] <- -2
# beta.syn[G/4 + 6:10,2] <- 2
# beta.syn[G/4 * 2 + 1:5,2] <- -2
# beta.syn[G/4 * 2 + 6:10,2] <- 2
# beta.syn[G/4 * 3 + 1:5,2] <- -2
# beta.syn[G/4 * 3 + 6:10,2] <- 2
#
# #the cell-type effects of the third cell type
# beta.syn[1:5,3] <- -2
# beta.syn[11:15, 3] <- 2
# beta.syn[G/4 + 1:5,3] <- -2
# beta.syn[G/4 + 11:15,3] <- 2
# beta.syn[G/4 * 2 + 1:5,3] <- -2
# beta.syn[G/4 * 2 + 11:15,3] <- 2
# beta.syn[G/4 * 3 + 1:5,3] <- -2
# beta.syn[G/4 * 3 + 11:15,3] <- 2
#
# #####################
# # Condition effects #
# #####################
...
# # Up-regulated or Down-regulated highly expressed genes in the corresponding cell types
# eta.syn[,K + 1:K] <- 0
# # for the first cell type
# eta.syn[1:3, K + 1] <- -1
# eta.syn[G/4 + 1:3,K + 1] <- 1
# eta.syn[G/4 * 2 + 16:17, K + 1] <- -1
# eta.syn[G/4 * 3 + 16:17, K + 1] <- 1
#
# # for the second cell type
# eta.syn[6:8, K + 2] <- -1
# eta.syn[G/4 + 6:8,K + 2] <- 1
# eta.syn[G/4 * 2 + 17:18,K + 2] <- -1
# eta.syn[G/4 * 3 + 17:18,K + 2] <- 1
#
# #################
# # batch effects #
# #################
...
# xi.syn[1,] <- c(0.5,0.35,0.15)
# xi.syn[2,] <- c(0.5,0.15,0.35)
# xi.syn[3,] <- c(0.5,0.35,0.15)
# xi.syn[4,] <- c(0.5,0.15,0.35)
# xi.syn <- xi.syn * 2000
#
# s_ind <- 0
# for(p in 1:P){
...
# }
#
# save.image("../DIFseq_test/True_parmeter.rdata")
#
# # Create a DIFseq object
...
# DIFseq_obj@iter$iter_steps_limit <- c(200,20)
# DIFseq_obj@iter$check_per_iter <- 1
# DIFseq_obj <- DIFseq_MCEM(DIFseq_obj, working_dir = "../DIFseq_test/")
#
# # Differential expression
# DIFseq_obj <- DiffExpression(DIFseq_obj)
#
# Intri_genes <- Intrinsic(DIFseq_obj)
# sum(Intri_genes)
#
...
# Celltype_color <- rainbow(3)
# BarChartProp(Cell_prop, Color = Celltype_color)
#
# # Batch 1 vs Batch 2 under Condition 1
# DIFseq_obj <- DiffAbundance(DIFseq_obj, ref = 1, dim = "Pair", subset = c(1,3))
#
# DiffAbun(DIFseq_obj)
#
# # Condition 1 vs Condition 2 under Batch 1
# DIFseq_obj <- DiffAbundance(DIFseq_obj, ref = 1, dim = "Pair", subset = c(1,2))
#
# DiffAbun(DIFseq_obj)
#
# # Condition 1 vs Condition 2 over both two batches
# DIFseq_obj <- DiffAbundance(DIFseq_obj, ref = 1, dim = "Cond", subset = c(1,2))
#
# DiffAbun(DIFseq_obj)
